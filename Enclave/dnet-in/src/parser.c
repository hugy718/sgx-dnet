...
// a set of operations equivalent to their above version with ocalls that work with memory buffer in enclave instead of untrusted mem or file.
// load_weights_upto(load_weights), save_weights_by_layer(save_weights) create_net_in

network *create_net_in_mem(list *sections)
{
    node *n = sections->front;
    if (n == NULL) return NULL; // sections empty
    network *net = make_network(sections->size - 1);
    ...
    net->input = calloc(net->inputs * net->batch, sizeof(float));
    net->truth = (net->input == NULL) ? NULL
      : calloc(net->truths * net->batch, sizeof(float));
     
    net->workspace = ((workspace_size) && (net->truth != NULL))
      ? calloc(1, workspace_size) : NULL;
    // if any error occurred we free the memory that net owns.  
